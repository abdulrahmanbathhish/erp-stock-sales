      ...
      <div class="sale-card__field">
        ...
        <div class="product-search-cell">
          <input type="text" 
                 ...
                 onkeydown="handleSearchKeyboard(event, '${rowId}')"
  ...
  searchTimeouts[rowId] = setTimeout(async () => {
    try {
      ...
      if (products.length === 0) {
        resultsDiv.innerHTML = `
          ...
          <button type="button" 
            ...
            <br>
            <small data-en="Name:" data-ar="الاسم:">الاسم:</small> ${highlightText(query, query)}
          </button>
        `;
        resultsDiv.classList.add('show');
        resultsDiv.style.display = 'block';
      ...
      resultsDiv.innerHTML = sortedProducts.map((product, index) => {
        ...
        const currentPrice = product.sale_price ? formatCurrency(product.sale_price) : '<span data-en="Not set" data-ar="غير محدد">غير محدد</span>';
        const latestSalePrice = product.latest_sale_price ? formatCurrency(product.latest_sale_price) : null;
        return `
        <button type="button" 
                  class="list-group-item list-group-item-action search-result-item" 
                  onclick="selectProduct('${rowId}', ${product.id}, '${product.name.replace(/'/g, "\\'")}', ${product.purchase_price}, ${product.sale_price || 'null'}, ${product.stock_quantity})"
                  data-row-id="${rowId}"
            ...
            <div class="d-flex justify-content-between align-items-start">
              <div class="flex-grow-1">
                ...
                <div class="search-result-item__details">
                  ...
                  ${latestSalePrice ? `<span class="text-muted">|</span><span class="text-info"><span data-en="Highest Sale:" data-ar="أعلى سعر بيع:">أعلى سعر بيع:</span> ${latestSalePrice}</span>` : ''}
                </div>
              </div>
            </div>
        </button>
...
// Select product
function selectProduct(rowId, productId, productName, purchasePrice, salePrice, stockQuantity) {
  ...
  // Store stock and product ID for validation
...
// Calculate row total (for credit sales, we show total amount, not profit)
...
function updateSummary() {
  ...
  document.getElementById('total-debt-preview').textContent = formatCurrency(totalDebt);
}
...
function selectCustomer(customerId, customerName) {
  ...
  resultsDiv.classList.remove('show');
  resultsDiv.style.display = 'none';
  document.getElementById('customer-warning').style.display = 'none';
}
...
// Submit all credit sales
async function submitAllCreditSales() {
  if (!selectedCustomerId) {
    document.getElementById('customer-warning').style.display = 'block';
    showAlert('Customer is required for credit sales', 'danger');
    return;
  }
  ...
  cards.forEach((card, index) => {
    ...
    // We need to fetch the product to check is_imported flag
    // For now, we'll allow negative stock if stock is 0 (likely a manually added product)
    // The server will handle the actual validation
    // But we'll still show a warning for imported products
  ...
  if (!confirm(`Submit ${sales.length} credit sale(s)? This will add debt to the customer.`)) {
    return;
  }
  
  try {
    showAlert('Processing credit sales...', 'info');
    ...
    if (!response.ok) {
      showAlert(data.error || 'Error creating credit sales', 'danger');
      return;
    }
    
    showAlert(`Successfully created ${data.sales_created} credit sale(s)! Debt added to customer.`, 'success');
    ...
    console.error('Error creating credit sales:', error);
    showAlert('Error creating credit sales', 'danger');
  }
}
...
document.addEventListener('click', (e) => {
  if (!e.target.closest('.product-search-cell')) {
    document.querySelectorAll('.search-results').forEach(div => {
      ...
      div.querySelectorAll('.active').forEach(item => item.classList.remove('active'));
    });
  }
  const resultsDiv = document.getElementById('customer-search-results');
...
// Close search results on Escape key globally
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    document.querySelectorAll('.search-results.show').forEach(div => {
      div.classList.remove('show');
      div.style.display = 'none';
      div.querySelectorAll('.active').forEach(item => item.classList.remove('active'));
    });
